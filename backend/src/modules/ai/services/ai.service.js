...
const getColumnName = (column) =>
  column?.name || column?.list_name || column?.title || "";
...
exports.generateInsights = async (userId, projectId) => {
  ...
  const tasks = await Task.find({
    ...
    .select(
      "title description priority status is_done story_point ai_suggested_point due_date board_id",
    )
    .lean();

  const boardMap = new Map(
    boards.map((board) => [String(board._id), board.name]),
  );
...
exports.summarizeTaskActivity = async (userId, taskId) => {
  ...
  if (!taskObjectId)
    throw new AppError("Task ID không hợp lệ.", 400, "BAD_REQUEST");

  const [task, comments, activities] = await Promise.all([
    Task.findById(taskObjectId).lean(),
    Comment.find({ task_id: taskObjectId })
      .sort({ created_at: -1 })
      .limit(30)
      .lean(),
    Activity.find({ task_id: taskObjectId })
      .sort({ created_at: -1 })
      .limit(30)
      .lean()
      .catch(() => []),
...
exports.generateSmartTasks = async (
  ...
  try {
    ...
    if (!boardObjectId || !projectObjectId)
      throw new AppError(
        "board_id hoặc project_id không hợp lệ.",
        400,
        "BAD_REQUEST",
      );

    const existingColumns = await Column.find({ board_id: boardObjectId })
      .sort({ created_at: 1 })
      .lean();
    ...
    const safeMemberIds = Array.isArray(memberIds)
      ? memberIds.map(toObjectId).filter(Boolean)
      : [];
    ...
    if (safeMemberIds.length > 0) {
      const users = await User.find({
        ...
        .select("_id full_name email team_id department_id")
        .lean();
      let index = 1;
      users.forEach((user) => {
...
FORMAT:
{
  ...
  "tasks": [
    ...
    const chatHistory = trimAiHistory(aiContext.messages).map((message) => ({
      role: message.role,
    ...
    for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
      ...
      try {
        ...
        if (
          !finalResponse.tasks ||
          !Array.isArray(finalResponse.tasks) ||
          finalResponse.tasks.length === 0
        ) {
    ...
    existingColumns.forEach((column) => {
      ...
      if (columnName)
        columnNameToIdMap[normalizeColumnKey(columnName)] = column._id;
    });
    ...
    if (newlyCreatedColIds.length > 0) {
      await Board.findByIdAndUpdate(boardObjectId, {
    ...
    if (!fallbackColumnId) {
      const fallbackColumn = await Column.create({
        board_id: boardObjectId,
        name: "TO DO",
      });
    ...
    for (const dto of finalResponse.tasks) {
      ...
      const assigneeId = dto.assignee_user_id
        ? aliasToRealId[dto.assignee_user_id]
        : null;
      ...
      const parentTask = new Task({
        ...
        ai_estimated_reason:
          dto.ai_estimation_reason || dto.ai_estimated_reason || "",
      ...
      if (Array.isArray(dto.subtasks) && dto.subtasks.length > 0) {
        for (const subDto of dto.subtasks) {
          const subAssigneeId = subDto.assignee_user_id
            ? aliasToRealId[subDto.assignee_user_id]
            : assigneeId;
    ...
    if (tasksToSave.length > 0) {
      ...
      const bulkColOps = Object.keys(colUpdates).map((columnId) => ({
        updateOne: {
          filter: { _id: columnId },
          update: {
            $push: { task_order_ids: { $each: colUpdates[columnId] } },
          },
        },
      }));
    ...
    aiContext.messages.push({
      role: "user",
      content: `Phát triển tiếp bảng với mode ${generationMode || "SIMPLE"}: ${userPrompt}`,
    });
...
exports.getDeviationInsights = async (projectId) => {
  ...
  const localEstimateTaskPoint = (task) => {
    ...
    if (
      /(database|schema|migration|rbac|permission|socket|realtime|payment|auth|security|deploy|docker|kubernetes|integration|api|backend)/i.test(
        text,
      )
    )
      score += 3;
    if (/(ui|form|button|style|layout|responsive|css|tailwind)/i.test(text))
      score += 1;
    if (/(bug|fix|hotfix|urgent|deadline|performance|optimize)/i.test(text))
      score += 2;
  ...
  const buildFallbackInsight = ({
    ...
    const taskInsights = tasks.map((task) => {
      ...
      const deviationPercent =
        suggested > 0 ? ((actual - suggested) / suggested) * 100 : 0;
    ...
    const aiTotal = taskInsights.reduce(
      (sum, task) => sum + Number(task.suggested_point || 0),
      0,
    );
    const totalDeviationPercent =
      aiTotal > 0 ? ((actualTotal - aiTotal) / aiTotal) * 100 : 0;

    let summaryStatus = "ACCURATE";
    let summaryComment =
      "AI fallback đã quét project bằng thuật toán nội bộ vì Gemini chưa trả được dữ liệu.";

    if (Math.abs(totalDeviationPercent) > 10 && totalDeviationPercent > 0) {
      summaryStatus = "UNDERESTIMATED";
      summaryComment =
        `AI fallback ước lượng thấp hơn thực tế ${totalDeviationPercent.toFixed(1)}%. ${reason || ""}`.trim();
    }

    if (Math.abs(totalDeviationPercent) > 10 && totalDeviationPercent < 0) {
      summaryStatus = "OVERESTIMATED";
      summaryComment =
        `AI fallback ước lượng cao hơn thực tế ${Math.abs(totalDeviationPercent).toFixed(1)}%. ${reason || ""}`.trim();
    }
  ...
  try {
    ...
    const [project, boards, tasks] = await Promise.all([
      ...
      Board.find({ project_id: projectObjectId, is_deleted: false })
        .select("_id name")
        .lean(),
      Task.find({
        project_id: projectObjectId,
        ...
        .select(
          "_id title description priority status is_done story_point board_id column_id assignees_user_id assignee_id",
        )
        .lean(),
    ]);
    ...
    const columns = await Column.find({
      ...
      .select("_id name board_id")
      .lean();

    const columnMap = new Map(
      columns.map((column) => [String(column._id), column.name]),
    );
...
Format bắt buộc:
{
  ...
  "tasks": [
    ...
    const aiTaskMap = new Map(
      aiTasks.map((task) => [String(task.task_id), task]),
    );

    let aiTotal = Number(aiData.project_ai_estimated_point || 0);

    if (!Number.isFinite(aiTotal) || aiTotal <= 0) {
      aiTotal = aiTasks.reduce(
        (sum, task) => sum + nearestFibonacci(task.ai_suggested_point),
        0,
      );
    }
    ...
    if (!Number.isFinite(aiTotal) || aiTotal <= 0) {
      return buildFallbackInsight({
        ...
        actualTotal,
        reason:
          "Gemini trả về tổng điểm không hợp lệ nên hệ thống dùng fallback.",
      });
    }

    const taskInsights = tasks.map((task) => {
      const aiTask = aiTaskMap.get(String(task._id));
      const suggested = aiTask
        ? nearestFibonacci(aiTask.ai_suggested_point)
        : localEstimateTaskPoint(task);
      const actual = Number(task.story_point || 0);
      const deviationPercent =
        suggested > 0 ? ((actual - suggested) / suggested) * 100 : 0;
      ...
      let status = "ACCURATE";
      let comment =
        aiTask?.reason || "AI ước tính gần với story point thực tế.";

      if (Math.abs(deviationPercent) > 10 && deviationPercent > 0) {
        status = "UNDERESTIMATED";
        comment =
          aiTask?.reason ||
          `AI ước lượng thấp hơn thực tế ${deviationPercent.toFixed(1)}%.`;
      }
      ...
      if (Math.abs(deviationPercent) > 10 && deviationPercent < 0) {
        ...
        comment =
          aiTask?.reason ||
          `AI ước lượng cao hơn thực tế ${Math.abs(deviationPercent).toFixed(1)}%.`;
      }
    ...
    let summaryComment =
      aiData.comment ||
      "Tổng ước lượng AI đang khá sát với tổng story point thực tế.";

    if (Math.abs(totalDeviationPercent) > 10 && totalDeviationPercent > 0) {
      summaryStatus = "UNDERESTIMATED";
      summaryComment =
        aiData.comment ||
        `AI đang ước lượng thấp hơn thực tế ${totalDeviationPercent.toFixed(1)}%.`;
    }
    ...
    if (Math.abs(totalDeviationPercent) > 10 && totalDeviationPercent < 0) {
      ...
      summaryComment =
        aiData.comment ||
        `AI đang ước lượng cao hơn thực tế ${Math.abs(totalDeviationPercent).toFixed(1)}%.`;
    }
