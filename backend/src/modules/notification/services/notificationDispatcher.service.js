...
const getProjectOwnerIdFromTask = async (task) => {
  ...
  try {
    ...
    const project = await Project.findById(projectId).select("owner_id").lean();
...
const resolveTaskManagerId = async (task, payload = {}) => {
  ...
  const validCandidates = candidates.map(toIdString).filter(Boolean);
...
const dispatch = async (
  ...
  const { emailEnabled, inAppEnabled, deadlineReminderEnabled } =
...
exports.dispatchTaskCreated = async (payload) => {
  ...
  const assignees = Array.isArray(task.assignees_user_id)
    ? task.assignees_user_id
    : [];
  const actorId =
    payload.userId || payload.senderId || task.author_user_id || null;
...
exports.dispatchTaskUpdated = async (payload) => {
  ...
  if (!task || !task.assignees_user_id || !task.assignees_user_id.length)
    return;
...
exports.dispatchTaskMoved = async (payload) => {
  ...
  const column = await Column.findById(
    payload.destColumnId || payload.dest_column_id,
  ).lean();
  const colName =
    column?.name || column?.list_name || column?.title || "another stage";
  const actorId = payload.userId || payload.senderId || null;
  const actionUrl = getTaskActionUrl(task);

  const metadata = buildTaskMetadata(task, {
    destination_column_id:
      payload.destColumnId || payload.dest_column_id || null,
...
exports.dispatchExtensionRequest = async (payload) => {
  ...
  const requester = await User.findById(
    payload.userId || payload.requesterId,
  ).lean();

  const requesterId = payload.userId || payload.requesterId;
  const requesterName = requester?.full_name || requester?.email || "Nhân viên";
  ...
  const htmlForManager = getBaseTemplate(
    "#6366F1",
  ...
  if (managerId) {
    ...
    console.warn(
      "[Notification] Không tìm thấy manager/project owner/admin để gửi yêu cầu dời deadline:",
      task._id,
    );
  }

  const htmlForRequester = getBaseTemplate(
    "#10B981",
...
exports.dispatchTaskCompleted = async (payload) => {
  ...
  const actorName = actor?.full_name || actor?.email || "Một thành viên";
