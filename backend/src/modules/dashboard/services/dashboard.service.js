...
const normalizeRoleName = (value) => String(value || '').trim().toUpperCase().replace(/\s+/g, '_').replace(/-/g, '_');
...
exports.getMetricsByRole = async (jwtUser, queryParams) => {
    ...
    const fallbackRole = normalizeRoleName(jwtUser.system_role || jwtUser.role_name || jwtUser.role || fullUser.system_role || fullUser.role_name || fullUser.role);
    if (fallbackRole && !roleNames.includes(fallbackRole)) roleNames.push(fallbackRole);
...
const getManagerProjectIds = async (managerId, departmentId) => {
    ...
    const [memberships, ownedProjects, departmentProjects] = await Promise.all([
        ProjectMember.find({ user_id: managerObjectId, is_active: true, is_deleted: false }).select('project_id').lean(),
        Project.find({ owner_id: managerObjectId, is_deleted: false }).select('_id').lean(),
        departmentObjectId
            ? Project.find({ department_id: departmentObjectId, is_deleted: false }).select('_id').lean()
            : Promise.resolve([])
    ]);
...
const buildTeamWorkload = async (departmentId, projectIds) => {
    ...
    const teams = departmentObjectId
        ? await Team.find({ department_id: departmentObjectId, is_deleted: false }).select('_id name code').sort({ name: 1 }).lean()
        : [];

    if (!departmentObjectId || projectIds.length === 0) return emptyManagerPayload(teams).team_workload_capacity;

    const memberCounts = await User.aggregate([
        { $match: { department_id: departmentObjectId, status: 'ACTIVE', team_id: { $ne: null } } },
        { $group: { _id: '$team_id', member_count: { $sum: 1 } } }
    ]);

    const memberCountMap = new Map(memberCounts.map((item) => [String(item._id), item.member_count]));
    ...
    const workloadRaw = await Task.aggregate([
        ...
        { $lookup: { from: 'users', localField: 'assignment_user_ids', foreignField: '_id', as: 'assignee' } },
        { $unwind: '$assignee' },
        {
            $match: {
    ...
    const workloadMap = new Map(workloadRaw.map((item) => [String(item._id), item]));
    return teams.map((team) => {
        ...
        const completionRate = totalPoints > 0 ? Math.round((completedPoints / totalPoints) * 1000) / 10 : 0;
        ...
        return {
            ...
            status: totalPoints === 0 ? 'NO_DATA' : completionRate >= 100 ? 'DONE' : 'IN_PROGRESS'
        };
    });
};
...
const buildDeadlineStatus = async (projectIds) => {
    ...
    const result = await TaskDeadline.aggregate([
        ...
        },
        {
            $group: {
                _id: null,
                overdue: { $sum: { $cond: [{ $or: [{ $eq: ['$is_overdue', true] }, { $lt: ['$due_date', now] }] }, 1, 0] } },
                at_risk: {
                    $sum: {
                        $cond: [
                            { $and: [{ $eq: ['$is_overdue', false] }, { $gte: ['$due_date', now] }, { $lte: ['$due_date', warningDate] }] },
                            1,
                            0
                        ]
                    }
                ...
                on_track: {
                    $sum: {
                        $cond: [
                            { $and: [{ $eq: ['$is_overdue', false] }, { $gt: ['$due_date', warningDate] }] },
                            1,
                            0
                        ]
                    }
    ...
    const raw = await TaskDeadline.aggregate([
        ...
        },
        {
            $addFields: {
                assignment_user_ids: {
                    $setUnion: [
                        { $cond: [{ $ne: ['$task.assignee_id', null] }, ['$task.assignee_id'], []] },
                        { $ifNull: ['$task.assignees_user_id', []] }
                    ]
                }
            }
        },
        { $lookup: { from: 'users', localField: 'assignment_user_ids', foreignField: '_id', as: 'assignees' } },
        { $lookup: { from: 'projects', localField: 'task.project_id', foreignField: '_id', as: 'project' } },
        { $unwind: { path: '$project', preserveNullAndEmptyArrays: true } },
        { $lookup: { from: 'boards', localField: 'task.board_id', foreignField: '_id', as: 'board' } },
...
const buildAiEfficiency = async (projectIds) => {
    ...
    const [projects, boardCounts, taskStats] = await Promise.all([
        Project.find({ _id: { $in: projectIds }, is_deleted: false }).select('_id name').lean(),
        Board.aggregate([
            { $match: { project_id: { $in: projectIds }, is_deleted: false } },
            { $group: { _id: '$project_id', boards_count: { $sum: 1 } } }
        ]),
        Task.aggregate([
            { $match: { is_deleted: false, project_id: { $in: projectIds }, story_point: { $gt: 0 } } },
            {
                $group: {
                    _id: '$project_id',
    ...
    const boardMap = new Map(boardCounts.map((item) => [String(item._id), item.boards_count]));
    ...
    const candidates = projectIds
        .map((projectId) => {
            ...
            return {
                ...
                score: boardsCount * 100000 + Number(stat.tasks_count || 0) * 100 + actualPoint
            };
        })
        .filter(Boolean);
...
const getManagerMetrics = async (userId, queryParams) => {
    const manager = await User.findById(userId).select('_id department_id team_id full_name').lean();
    const departmentId = manager?.department_id;
    const departmentTeams = departmentId
        ? await Team.find({ department_id: departmentId, is_deleted: false }).select('_id name code').sort({ name: 1 }).lean()
        : [];
    ...
    const [teamWorkload, deadlineStats, atRiskTasks, aiEfficiency] = await Promise.all([
...
const getMemberMetrics = async (userId, queryParams) => {
        ...
        { $unwind: { path: '$dl', preserveNullAndEmptyArrays: true } },
        {
            $group: {
                ...
                tasks_completed_this_week: {
                    $sum: {
                        $cond: [
                            { $and: [{ $eq: ['$is_done', true] }, { $gte: ['$dl.actual_completed_at', sevenDaysAgo] }] },
                            1,
                            0
                        ]
                    }
                ...
                on_time_completed: { $sum: { $cond: [{ $eq: ['$dl.completion_status', 'ON_TIME'] }, 1, 0] } }
            }
        }
    ]);
    ...
    const myFocusBoard = tasks.map((task) => {
        const deadline = deadlines.find((item) => String(item.task_id) === String(task._id));
        let status = 'ON_TRACK';

        if (deadline) {
            status = (deadline.is_overdue || new Date(deadline.due_date) < now)
                ? 'OVERDUE'
                : (new Date(deadline.due_date) < new Date(now.getTime() + 24 * 60 * 60 * 1000) ? 'AT_RISK' : 'ON_TRACK');
        }
