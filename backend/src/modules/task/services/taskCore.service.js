...
const assertAssignableUsers = async (userIds = [], actorId = null) => {
  const ids = Array.from(
    new Set(userIds.map((id) => toIdString(id)).filter(Boolean)),
  );
  ...
  const protectedUser = users.find((user) => {
    ...
    return !(
      context.actorIsSystemAdmin && String(user._id) === String(context.actorId)
    );
  });
...
const getProjectIdFromBoard = async (boardId) => {
  ...
  const board = await Board.findById(boardId).select("project_id").lean();
...
const normalizeTaskPayload = async (taskData = {}, actorId = null) => {
  ...
  if (!payload.column_id) {
    throw new AppError(
      "Missing required field: column_id",
      400,
      "VALIDATION_FAILED",
    );
  }
  ...
  payload.assignee_id =
    payload.assignee_id ||
    (payload.assignees_user_id.length > 0
      ? payload.assignees_user_id[0]
      : null);
...
const normalizeTaskUpdatePayload = async (
  updateData = {},
  oldTask = null,
  actorId = null,
) => {
  ...
  const wantsDone = payload.is_done === true || payload.status === "DONE";

  const wantsUndone =
    payload.is_done === false || (payload.status && payload.status !== "DONE");
...
const updateDeadlineCompletionStatus = async (taskId, task, completedAt) => {
  ...
  const dueDate = deadlineRecord?.due_date || task?.due_date || null;
  ...
  const isLate =
    dueDate && new Date(completedAt).getTime() > new Date(dueDate).getTime();
...
exports.createTask = async (taskData, actorId = null) => {
  ...
  await Column.findByIdAndUpdate(payload.column_id, {
    $push: {
...
exports.updateTask = async (taskId, updateData, actorId = null) => {
  ...
  const payload = await normalizeTaskUpdatePayload(
    updateData,
    oldTask,
    actorId,
  );
