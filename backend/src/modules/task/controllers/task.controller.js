    ...
    if (!taskData || Object.keys(taskData).length === 0) {
      return res.status(400).json({
        ...
        error: {
          ...
          message:
            "Không nhận được dữ liệu. Vui lòng kiểm tra lại phía Frontend.",
        },
      });
    }
...
exports.updateTask = async (req, res, next) => {
  try {
    const actorId = getAuthUserId(req);
    const task = await taskCoreService.updateTask(
      req.params.id,
      req.body,
      actorId,
    );
...
exports.addMultipleSubtasks = async (req, res, next) => {
  try {
    const task = await subtaskService.addMultipleSubtasks(
      req.params.id,
      req.body.titles,
    );
...
exports.deleteSubtask = async (req, res, next) => {
  try {
    const task = await subtaskService.deleteSubtask(
      req.params.id,
      req.params.subtaskId,
    );
  ...
  try {
    ...
    res.status(200).json({
      ...
      message: comment.is_resolved
        ? "Đã đánh dấu bình luận là đã giải quyết."
        : "Đã bỏ đánh dấu đã giải quyết.",
    });
  } catch (error) {
    next(error);
  }
