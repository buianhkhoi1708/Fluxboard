...
exports.requestExtension = async (taskId, userId, newDueDate, reason) => {
  ...
  if (isInvalidDate(newDueDate)) {
    throw new AppError(
      "Deadline mới không hợp lệ hoặc chưa được cung cấp.",
      400,
    );
  }
  ...
  if (parsedNewDueDate <= currentDueDate) {
    throw new AppError(
      "Bạn đã gửi yêu cầu dời hạn trước đó. Vui lòng chờ quản lý xử lý.",
      400,
    );
  }
...
exports.sendDelayedNotification = (
  user,
  task,
  deadlineRecord,
  delayMinutes = 10,
...
exports.dispatchTaskDeadlineNotification = async (
  user,
  task,
  deadlineRecord,
) => {
  ...
  const { emailEnabled, pushEnabled, deadlineReminderEnabled } =
  ...
  const formattedDate = new Date(deadlineRecord.due_date).toLocaleString(
    "vi-VN",
  );
