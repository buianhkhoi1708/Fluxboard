...
exports.getSecurityActivities = async ({
    ...
    const query = {
        $or: [
            { source: 'SECURITY' },
            { action: { $in: ['CREATE_USER', 'CHANGE_PASSWORD', 'REVOKE_ACCESS', 'LOGIN', 'LOGOUT'] } },
        ],
    };
...
/**
 * Compatibility layer:
 * Một số service cũ vẫn emit eventBus.emit('activity_log', ...)
 * với format actor_user_id/source_type/message.
 * Đoạn này giúp log cũ vẫn được ghi vào Activity schema mới.
 */
