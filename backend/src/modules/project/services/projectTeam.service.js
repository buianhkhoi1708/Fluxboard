...
exports.assignProjectToTeam = async (
  projectId,
  teamId,
  defaultRoleName = Roles.MEMBER,
) => {
  ...
  // Lấy User thuộc Team
  const usersInTeam = await User.find({ team_id: teamId, status: "ACTIVE" })
    .select("_id")
    .lean();
  if (usersInTeam.length === 0)
    return { message: "No active users found in this team to assign" };

  // Lọc User chưa có trong Project
  const existingMembers = await ProjectMember.find({ project_id: projectId })
    .select("user_id")
    .lean();
  const existingUserIds = existingMembers.map((m) => m.user_id.toString());

  const newUsersToAssign = usersInTeam.filter(
    (u) => !existingUserIds.includes(u._id.toString()),
  );

  if (newUsersToAssign.length === 0)
    return { message: "All team members are already in the project" };
