...
const toObjectId = (value) => {
    ...
    const id = typeof value === 'object'
        ? value._id || value.id || value.user_id
        : value;
...
const getActor = async (actorId) => {
    ...
    return User.findById(actorObjectId)
...
const getActorVisibilityContext = async ({
    ...
    /**
     * Rule:
     * - Non-SYSTEM_ADMIN: không bao giờ thấy SYSTEM_ADMIN.
     * - SYSTEM_ADMIN bình thường: xem được toàn bộ nếu không yêu cầu exclude.
     * - SYSTEM_ADMIN trong các dropdown chọn user: chỉ thấy chính mình nếu includeCurrentSystemAdmin=true.
     */
    const shouldHideSystemAdmin =
        !actorIsSystemAdmin || excludeSystemAdmin;
...
exports.getUserById = async (userId, actorId) => {
    ...
    return normalizeUserForResponse(
        target,
        sessionMap.get(String(target._id)),
    );
};
...
exports.getAllUsers = async ({
    ...
    const [totalElements, users] = await Promise.all([
        ...
        User.find(finalFilter)
            .select('-password -password_hash -reset_password_token -reset_password_expires')
            .populate('role_id', 'name scope')
    ...
    const normalizedUsers = users.map((user) => {
        return normalizeUserForResponse(
            user,
            sessionMap.get(String(user._id)),
        );
    });
