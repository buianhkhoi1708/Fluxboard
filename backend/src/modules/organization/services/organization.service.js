...
const sanitizeDepartment = (department, context) => {
  ...
  const sanitizedManager = sanitizeUserForResponse(
...
const buildUserVisibilityFilter = async (context, baseFilter = {}) => {
  ...
  return {
    $and: [baseFilter, nonSystemAdminFilter],
  };
};

const userSafeSelect =
  "_id full_name email avatar_url role_id status team_id department_id";
...
exports.createDepartment = async (
  { name, code, parent_id, description, manager_id },
  actorUser,
) => {
  if (!name || !code) {
...
exports.updateDepartment = async (id, updateData, actorUser) => {
  ...
  const updatedDept = await Department.findByIdAndUpdate(id, updateData, {
...
exports.createTeam = async (
  { name, code, department_id, description, lead_id },
  actorUser,
) => {
