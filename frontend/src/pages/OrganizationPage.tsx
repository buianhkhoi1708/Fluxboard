...
} from "../features/organization/types/orgTypes";
...
const OrganizationPage: React.FC<OrganizationPageProps> = () => {
  ...
  const handleDeleteDepartment = async (deptId: string, deptName: string) => {
    if (
      !window.confirm(
        `⚠️ CẢNH BÁO: Bạn có chắc chắn muốn xóa phòng ban "${deptName}" không?\nTất cả các Team bên trong cũng có thể bị ảnh hưởng.`,
      )
    )
      return;
    try {
      ...
      alert(
        error.response?.data?.message || "Có lỗi xảy ra khi xóa phòng ban.",
      );
    }
  };
  ...
  const handleDeleteTeam = async (teamId: string, teamName: string) => {
    if (
      !window.confirm(
        `Bạn có chắc chắn muốn xóa Team "${teamName}" không?\nCác thành viên trong team sẽ trở về trạng thái chưa gán nhóm.`,
      )
    )
      return;
    try {
  ...
  const handleRemoveMember = async (
    ...
    if (
      !window.confirm(
        `Bạn có chắc chắn muốn gỡ nhân sự "${userName}" khỏi Team này?`,
      )
    )
      return;
    try {
          ...
          <div className="space-y-10 pb-20">
            {orgTree.map((dept: any) => {
              ...
              return (
                <div
                  key={departmentId}
                  className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-lg shadow-slate-200/20 border border-slate-200/80 overflow-hidden transition-all hover:shadow-xl hover:shadow-indigo-100/20 hover:border-indigo-200/60"
                >
                  {}
                    ...
                    <div className="flex items-center gap-5">
                      ...
                      <div className="group/dept relative">
                        <div className="flex items-center gap-3">
                          ...
                          <button
                            onClick={() =>
                              openEditDeptModal(dept, displayManagerName)
                            }
                          ...
                          <button
                            onClick={() =>
                              handleDeleteDepartment(departmentId, dept.name)
                            }
                            title="Xóa Phòng ban"
                    ...
                    {dept.teams && dept.teams.length > 0 ? (
                      dept.teams.map((team: any) => {
                        ...
                        return (
                          <div
                            key={teamIdToUse}
                            className="group/team bg-white/90 backdrop-blur-sm border border-slate-200 rounded-2xl p-5 md:p-6 shadow-sm hover:shadow-md hover:shadow-indigo-100/20 hover:border-indigo-300 transition-all duration-200 relative"
                          >
                            <div className="flex justify-between items-start mb-5">
                              ...
                              <div className="flex items-center gap-1 opacity-0 group-hover/team:opacity-100 transition-all">
                                ...
                                <button
                                  onClick={() =>
                                    handleDeleteTeam(teamIdToUse, team.name)
                                  }
                                  title="Xóa Team"
                              ...
                              {team.members && team.members.length > 0 ? (
                                team.members.map((member: any) => {
                                  const memberId =
                                    ...
                                    member.user_id;
                                  const memberName =
                                    member.full_name || member.fullName || "U";

                                  return (
                                    <div
                                      key={memberId}
                                      className="group/member flex justify-between items-center px-3 py-2.5 rounded-xl hover:bg-slate-50 transition-colors border border-transparent hover:border-slate-100 relative"
                                    >
                                      <div className="flex items-center gap-3">
                                        ...
                                        <div>
                                          <p className="text-sm font-semibold text-slate-700 group-hover/member:text-indigo-700 transition-colors">
                                            {memberName}
                                          </p>
                            ...
                            <button
                              onClick={() =>
                                openAddMemberModal(departmentId, teamIdToUse)
                              }
                      ...
                      <div className="col-span-full py-12 text-center bg-white/60 backdrop-blur-sm border border-dashed border-slate-200 rounded-2xl">
                        <p className="text-slate-400 font-medium">
                          Phòng ban này đang trống, hãy bắt đầu bằng việc tạo
                          một Team.
                        </p>
                      </div>
                    )}
