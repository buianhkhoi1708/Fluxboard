...
const StepIndicator = ({ currentStep }) => (
  ...
  >
    {[1, 2, 3].map((step) => (
      <div
        key={step}
        className={`h-1.5 rounded-full transition-all duration-500 ${currentStep >= step ? "w-6 bg-indigo-500 shadow-sm" : "w-2 bg-slate-200"}`}
      />
    ))}
  </div>
);
...
const WorkspaceSkeleton = () => (
  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
    {[1, 2, 3].map((i) => (
      <div
        key={i}
        className="p-5 rounded-2xl bg-white border border-slate-100 animate-pulse"
      >
...
const AiBoardGeneratorPage = () => {
  ...
  const viewerRoleId = useMemo(() => {
    ...
    const projectRoles = systemRoles.filter((r) =>
  ...
  useEffect(() => {
    if (projectMembers.length > 0) {
      const defaultSelections = projectMembers.map((m: any) => ({
        ...
        roleId:
          (m.roleIds && m.roleIds[0]) || (m.role_ids && m.role_ids[0]) || "",
      }));
  ...
  const nextStep = () => {
    ...
    if (currentStep === 2 && selectedMembers.length === 0)
      return setErrors({
        members: "Cần tích chọn ít nhất 1 nhân sự để đưa vào Board!",
      });
    setErrors({});
  ...
  const handleFinalGenerate = async () => {
    ...
    try {
      ...
      const errorMsg =
        ...
        e.message ||
        "Hệ thống quá tải hoặc hết Token";
                ...
                {projectList.length === 0 ? (
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 max-h-[60vh] overflow-y-auto custom-scrollbar content-start pb-4">
                    ...
                    >
                      ...
                      <span className="text-xs font-bold text-slate-400 group-hover:text-indigo-600 uppercase tracking-widest">
                        Tạo mới
                      </span>
                    </button>
                    {projectList.map((item: any) => {
                      ...
                      return (
                        <button
                          ...
                          className={`p-5 rounded-2xl border-2 text-left transition-all relative ${isSelected ? "border-indigo-600 bg-white shadow-xl shadow-indigo-100/50" : "border-white bg-white shadow-sm hover:border-slate-200 hover:shadow-md"}`}
                        >
                          {isSelected && (
                          ...
                          <h3 className="font-bold text-slate-800 text-sm truncate pr-6">
                            {p.name}
                          </h3>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      {projectMembers.map((member: any) => {
                        ...
                        if (roleObj) {
                          ...
                          (rawRole.name || rawRole.code)
                        ) {
                          roleName = rawRole.name || rawRole.code;
                        }

                        const isSelected = !!selectedMembers.find(
                          (m) => m.userId === safeUserId,
                        );
                        ...
                        return (
                          ...
                          >
                            ...
                            >
                              <div
                                className={`w-10 h-10 rounded-xl flex items-center justify-center font-black text-sm shrink-0 border ${isSelected ? "bg-indigo-600 text-white border-indigo-700" : "bg-slate-100 text-slate-500 border-slate-200"}`}
                              >
                                {name.charAt(0).toUpperCase()}
                              </div>
                              <div className="flex-1 min-w-0">
                                <div
                                  className={`font-bold text-sm truncate transition-colors ${isSelected ? "text-indigo-950" : "text-slate-800"}`}
                                >
                                  {name}
                                </div>
                                <div className="flex items-center gap-2 mt-0.5">
                                  ...
                                  <span
                                    className={`text-[9px] font-black uppercase tracking-wider px-1.5 py-0.5 rounded ${isSelected ? "bg-indigo-100 text-indigo-700" : "bg-slate-100 text-slate-500"}`}
                                  >
                                    {roleName}
                                  </span>
                                </div>
                            ...
                            <div className="flex items-center gap-2 shrink-0">
                              ...
                              <div
                                onClick={() => toggleMember(safeUserId, roleId)}
                                className={`w-5 h-5 rounded-md border-2 flex items-center justify-center transition-all cursor-pointer ${isSelected ? "border-indigo-600 bg-indigo-600" : "border-slate-300 bg-white"}`}
                              >
                                {isSelected && (
                                  <CheckCircle2
                                    size={12}
            ...
            {currentStep === 3 && (
              <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-500">
                <div>
                  ...
                  <p className="text-sm text-slate-500 font-medium mt-1">
                    Mô tả quy trình để AI tự động phân rã task và tính toán
                    Story Points.
                  </p>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  <button
                    ...
                    className={`p-4 rounded-2xl border-2 flex items-center gap-4 transition-all ${generationMode === "SIMPLE" ? "border-indigo-600 bg-white shadow-xl shadow-indigo-100" : "bg-white border-slate-100 opacity-70 hover:opacity-100"}`}
                  >
                    <div
                      className={`p-2.5 rounded-xl ${generationMode === "SIMPLE" ? "bg-indigo-600 text-white" : "bg-slate-100"}`}
                    >
                      <Layout size={20} />
                    </div>
                  ...
                  <button
                    ...
                    className={`p-4 rounded-2xl border-2 flex items-center gap-4 transition-all ${generationMode === "ADVANCED" ? "border-indigo-600 bg-white shadow-xl shadow-indigo-100" : "bg-white border-slate-100 opacity-70 hover:opacity-100"}`}
                  >
                    <div
                      className={`p-2.5 rounded-xl ${generationMode === "ADVANCED" ? "bg-indigo-600 text-white" : "bg-slate-100"}`}
                    >
                      <Layers size={20} />
                    </div>
                    <div className="text-left">
