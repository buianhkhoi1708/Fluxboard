...
export const ManagerDashboardSkeleton = () => (
  <div className="grid grid-cols-1 lg:grid-cols-3 gap-5 pb-8 animate-pulse">
    {[...Array(3)].map((_, i) => (
      <div
        key={i}
        className="bg-white/80 backdrop-blur-sm rounded-2xl border border-slate-200/80 p-5 lg:h-[calc(100vh-210px)] lg:max-h-[520px] min-h-[390px] space-y-4"
      >
...
const ManagerDashboard = ({ data }: ManagerDashboardProps) => {
  ...
  const {
    ...
    isError: isAiError,
    refetch: refetchAi,
  } = useAiDeviationInsights(aiProjectId);

  const workloadSummary = useMemo(() => {
    return teamWorkload.reduce(
      (acc, team) => {
  ...
  const aiChartData = useMemo(() => {
    ...
    const projectName =
      aiInsight?.project_name || aiProject?.project_name || "Project";
    return [
      {
        name: projectName,
        ai_suggested_point: Number(aiInsight?.ai_suggested_point || 0),
        actual_point: Number(
          aiInsight?.actual_point || aiProject?.actual_point || 0,
        ),
        tasks_count: Number(
          aiInsight?.tasks_count || aiProject?.tasks_count || 0,
        ),
        ...
        <div className="flex justify-between items-start mb-4 shrink-0">
          <div>
            ...
            <p className="text-xs text-slate-500 font-medium mt-1">
              Story points hoàn thành / tổng theo team
            </p>
          </div>
            ...
            {teamWorkload.map((team, idx) => {
              ...
              return (
                <div
                  ...
                  className="rounded-2xl border border-slate-200 bg-white p-3.5 shadow-sm hover:border-indigo-200 hover:shadow-md transition-all"
                >
                    ...
                    <div className="min-w-0">
                      <div className="flex items-center gap-2">
                        ...
                        <h4 className="text-sm font-black text-slate-800 truncate">
                          {team.team_name || team.team_code || "Team"}
                        </h4>
                      </div>
                  ...
                  <div className="mt-2 flex justify-between text-[11px] font-bold text-slate-400">
                    ...
                    <span>
                      {total === 0 ? "Chưa có estimate" : "Có story point"}
                    </span>
                  </div>
                </div>
              );
        ...
        <div className="flex justify-between items-start mb-4 shrink-0">
          <div>
            ...
            <p className="text-xs text-rose-500 font-bold mt-1 flex items-center gap-1.5">
              <ShieldAlert size={14} strokeWidth={2.5} /> Top 10 task gần hạn
              ...
              {atRiskTasks.map((task, idx) => {
                ...
                return (
                  <button
                    ...
                    className="w-full text-left p-3.5 bg-white border border-rose-100 shadow-sm rounded-xl hover:shadow-md hover:border-rose-300 hover:-translate-y-0.5 transition-all group"
                  >
                    <div className="flex justify-between items-start gap-3 mb-2.5">
                      <h4 className="text-sm font-bold text-slate-800 line-clamp-1 leading-snug group-hover:text-rose-600 transition-colors">
                        {task.title}
                      </h4>
                      <span
                        className={`shrink-0 text-[10px] font-black px-2.5 py-1 rounded-lg uppercase tracking-wider border ${isOverdue ? "bg-rose-50 text-rose-600 border-rose-100" : "bg-amber-50 text-amber-600 border-amber-100"}`}
                      >
                      ...
                      <div className="flex items-center -space-x-2 min-h-[28px]">
                        {assignees.length === 0 ? (
                          ...
                          <>
                            {assignees.slice(0, 4).map((user, userIdx) => {
                              ...
                              return user.avatar_url ? (
                                ...
                                <div
                                  key={user.user_id || userIdx}
                                  title={name}
                                  className="w-7 h-7 rounded-full border-2 border-white bg-indigo-600 text-white flex items-center justify-center text-[10px] font-black shadow-sm"
                                >
                                  {getInitial(name)}
                                </div>
                              );
                            ...
                            {assignees.length > 4 && (
                              <div className="w-7 h-7 rounded-full border-2 border-white bg-slate-100 text-slate-500 flex items-center justify-center text-[10px] font-black">
                                +{assignees.length - 4}
                              </div>
                            )}
                          </>
      ...
      <div className="bg-white/80 backdrop-blur-sm rounded-2xl border border-slate-200/80 shadow-lg shadow-slate-200/20 p-5 flex flex-col lg:h-[calc(100vh-210px)] lg:max-h-[520px] min-h-[390px] transition-all hover:shadow-xl hover:shadow-indigo-100/20">
        <div className="flex justify-between items-start mb-4 shrink-0">
          <div>
            ...
            <p className="text-xs text-slate-500 font-medium mt-1">
              AI quét project rồi so với story point thực tế
            </p>
          </div>
        ...
        {!aiProjectId ? (
          <div className="flex-1 flex items-center justify-center text-sm font-bold text-slate-300 bg-slate-50/50 rounded-2xl border border-dashed border-slate-200">
            AI chưa có project đủ dữ liệu để quét
          </div>
        ) : isAiLoading ? (
          <div className="flex-1 flex flex-col items-center justify-center text-center bg-slate-50/50 rounded-2xl border border-dashed border-slate-200 px-6">
            ...
            <p className="text-sm font-black text-slate-700">
              AI đang quét bảng và ước tính effort...
            </p>
          ...
          <div className="flex-1 flex flex-col items-center justify-center text-center bg-rose-50/50 rounded-2xl border border-dashed border-rose-200 px-6">
            ...
            <p className="text-sm font-black text-slate-700">
              AI chưa quét được dữ liệu
            </p>
            <button
              type="button"
              onClick={() => refetchAi()}
              className="mt-4 rounded-xl bg-rose-600 px-4 py-2 text-xs font-black text-white hover:bg-rose-700 transition"
            >
              Quét lại
            </button>
          </div>
          ...
          <>
            <div className="shrink-0 mb-3 rounded-2xl bg-slate-50 border border-slate-100 p-3">
              <p className="text-[10px] font-black uppercase tracking-widest text-slate-400">
                Project được AI quét
              </p>
              <p className="mt-1 text-sm font-black text-slate-800 truncate">
                {aiChartData[0]?.name}
              </p>
              <p className="mt-1 text-[11px] font-bold text-slate-400">
                {aiChartData[0]?.boards_count || 0} board ·{" "}
                {aiInsight?.scanned_tasks_count ||
                  aiChartData[0]?.tasks_count ||
                  0}{" "}
                task có story point
              </p>
            </div>
            ...
            <div className="shrink-0 mb-2 flex flex-wrap justify-center gap-4 bg-white py-2 rounded-xl border border-slate-100 shadow-sm">
              <div className="flex items-center gap-2 text-[11px] font-bold text-slate-600 uppercase tracking-wider">
                <div className="w-3 h-3 rounded-md bg-indigo-500 shadow-sm" />{" "}
                AI ước tính
              </div>
              <div className="flex items-center gap-2 text-[11px] font-bold text-slate-600 uppercase tracking-wider">
                <div className="w-3 h-3 rounded-md bg-emerald-400 shadow-sm" />{" "}
                Thực tế chốt
              </div>
            </div>
