...
const ProjectSettingsTab = ({ projectId }) => {
  ...
  const { mutateAsync: updateProject, isPending: isUpdating } =
    useUpdateProjectInfo(projectId);
  ...
  const handleDelete = async () => {
    ...
    const confirmName = prompt(
      `CẢNH BÁO: Hành động này sẽ xóa vĩnh viễn dự án và toàn bộ Task bên trong!\n\nGõ "${currentProject.name}" để xác nhận xóa:`,
    );
        ...
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1">
            ...
            <p className="text-sm text-rose-600/80 mt-1 mb-4 font-medium">
              Xóa dự án sẽ xóa vĩnh viễn toàn bộ Bảng (Boards), Cột, và Công
              việc (Tasks) bên trong. Hành động này KHÔNG THỂ hoàn tác!
            </p>
            <button
            ...
            >
              <Trash2 size={16} />{" "}
              {isDeleting ? "Đang xóa..." : "Xóa vĩnh viễn Dự án"}
            </button>
          </div>
        </div>
      </div>
