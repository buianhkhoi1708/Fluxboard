...
const ProjectDetailMemberList = ({
  ...
  const getRoleName = (roleId: string) => {
    const role = systemRoles.find(
      (r: any) => r.id === roleId || r.code === roleId || r.name === roleId,
    );
  ...
  const handleKickMember = async (userId: string, memberName: string) => {
    ...
    if (
      window.confirm(
        `Sếp có chắc chắn muốn kick "${memberName}" ra khỏi dự án này không?`,
      )
    ) {
      try {
          ...
          <tbody className="divide-y divide-slate-100">
            {filteredMembers.length > 0 ? (
              filteredMembers.map((m: any, idx: number) => {
                // 🚀 ĐÂY LÀ CHỖ CẦN SỬA ĐỂ LẤY ĐÚNG DỮ LIỆU TỪ user_id
                const userData = m.user_id || {};

                // Lấy ID: ưu tiên _id của user_id, nếu không thì lấy ID của member
                const safeUserId = userData._id || m.userId || m._id || idx;

                // Lấy Name, Email, Avatar từ userData (object user_id)
                const name = userData.full_name || m.name || "Unnamed User";
                const email = userData.email || m.email || "Chưa có email";
                const avatar = userData.avatar_url || m.avatar_url;

                // Trạng thái: dùng is_active từ dữ liệu JSON của Sếp
                const isActive = m.is_active !== false;

                // Roles: lấy từ role_ids
                const roles = m.role_ids || [];
                    ...
                    <td className="px-6 py-4">
                      <div className="flex flex-wrap gap-2">
                        {roles.map((role: any, index: number) => {
                          // Kiểm tra nếu role là object thì lấy .name, nếu là string thì lấy trực tiếp
                          const roleKey =
                            typeof role === "string" ? role : role._id || index;
                          const roleDisplay =
                            typeof role === "string"
                              ? getRoleName(role)
                    ...
                    <td className="px-6 py-4">
                      {isActive ? (
                        ...
                        <span className="inline-flex items-center gap-1.5 text-[11px] font-bold text-slate-600 bg-slate-100 border border-slate-200 px-3 py-1.5 rounded-full">
                          <XCircle size={14} className="text-slate-400" /> Vô
                          hiệu hóa
                        </span>
                      )}
                    </td>
