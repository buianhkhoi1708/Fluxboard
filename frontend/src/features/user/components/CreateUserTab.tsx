...
const CreateUserTab: React.FC = () => {
  ...
  useEffect(() => {
    const fetchRoles = async () => {
      try {
        ...
        const roleData = extractRoles(res).filter(
        ...
        setErrorMsg(
          "Không thể tải danh sách quyền hạn. Vui lòng tải lại trang.",
        );
      } finally {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (selectedRole && isSystemAdminRole(selectedRole)) {
      setErrorMsg(
        "Không được cấp role SYSTEM_ADMIN từ giao diện tạo tài khoản.",
      );
      setIsSubmitting(false);
      return;
    }
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
          <div className="space-y-1">
            ...
            <p className="text-sm font-medium text-slate-500 pl-12">
              Tạo tài khoản và phân quyền cho nhân sự mới. Role SYSTEM_ADMIN
              được bảo vệ và không hiển thị tại đây.
            </p>
          </div>
        </div>
